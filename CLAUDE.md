...
- Fixed (`1d224fc`): `AlarmView.onShow()` and `NappingView.onShow()` both unconditionally reset
  session state (hard-cap clock, baseline capture, progress display / nap start time, initial
  peek window) on every show, with no guard against the OS re-showing an already-active view
  (e.g. a notification overlay) without destroying it. Both now guard the one-time state behind
